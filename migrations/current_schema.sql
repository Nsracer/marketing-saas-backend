...
CREATE TABLE public.competitor_cache (
  ...
  user_instagram_handle character varying,
  user_facebook_handle character varying,
  user_linkedin_handle character varying,
  competitor_instagram_handle character varying,
  competitor_facebook_handle character varying,
  competitor_linkedin_handle character varying,
...
CREATE TABLE public.reports (
  ...
  user_id uuid NOT NULL,
  user_email text NOT NULL,
  report_type text NOT NULL CHECK (report_type = ANY (ARRAY['dashboard'::text, 'competitor'::text, 'social'::text, 'seo'::text, 'overall'::text])),
  ...
  status text DEFAULT 'generated'::text CHECK (status = ANY (ARRAY['generating'::text, 'generated'::text, 'failed'::text, 'expired'::text])),
  error_message text,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
...
CREATE TABLE public.social_media_cache (
  ...
  linkedin_company_urn character varying,
...
CREATE TABLE public.users_table (
  ...
  plan text NOT NULL DEFAULT 'free'::text CHECK (plan = ANY (ARRAY['free'::text, 'starter'::text, 'growth'::text, 'pro'::text])),
  stripe_id text DEFAULT ''::text,
  subscription_status text DEFAULT 'active'::text CHECK (subscription_status = ANY (ARRAY['active'::text, 'cancelled'::text, 'past_due'::text, 'trialing'::text, 'incomplete'::text, 'incomplete_expired'::text])),
...
CREATE TABLE public.website_analysis_cache (
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT website_analysis_cache_pkey PRIMARY KEY (id)
);