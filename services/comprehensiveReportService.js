...
class ComprehensiveReportService {
  ...
  addCoverPage(doc, data) {
    ...
    try {
      if (fs.existsSync(LOGO_PATH)) {
        doc.image(LOGO_PATH, 240, 40, { width: 80, height: 80 });
      }
    } catch (error) {
  ...
  addSEOExecutiveSummary(doc, data) {
    ...
    if (lighthouse && lighthouse.categoryScores) {
      ...
      if (data.healthScore && (data.healthScore.overall_score !== undefined || data.healthScore.overall !== undefined)) {
        avgScore = data.healthScore.overall_score ?? data.healthScore.overall;
        console.log('📄 Using official Health Score for PDF:', avgScore);
      } else {
        // Fallback to simple average if no official health score provided
        avgScore = Math.round(
        ...
        console.log('📄 Using calculated average for PDF:', avgScore);
      }
